...
// Singleton
// When we only want a module to be instantied one
...
// look that the singleton it's private
...
func GetSingletonDatabase() Database {
	once.Do(func() {
		...
		caps, err := readData("./2-creational/singleton/capitals.txt")
		if err == nil {
			db.capitals = caps
		}
		instance = &db
...
// this breaks the dependency inversion principle cause we are reliying on live data
// we want to rely on abstractions
// then we can test
